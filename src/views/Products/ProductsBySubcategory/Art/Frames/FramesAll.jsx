...
const FramesAll = () => {
  ...
  const handleSortByLowerPrice = () => {
    setProduct(
      [...activeAuction].sort((a, b) => a.initialPrice - b.initialPrice)
    );
  };

  const handleSortByHighestPrice = () => {
    setProduct(
      [...activeAuction].sort((a, b) => b.initialPrice - a.initialPrice)
    );
  };
  ...
  return (
    <div>
      <div className="dropdown">
        <button
          className="btn btn-secondary dropdown-toggle m-3 mb-4 ml-4 btn-filter"
          type="button"
        ...
        <div className="dropdown-menu" aria-labelledby="dropdownMenuButton">
          <a className="dropdown-item" onClick={handleSortByLowerPrice}>
            Order by Lower Price
          </a>
          <a className="dropdown-item" onClick={handleSortByHighestPrice}>
            {" "}
            Order by Highest Price
          </a>
          <a className="dropdown-item" onClick={handleSortByNewest}>
            {" "}
            Order by Newest
          </a>
          <a className="dropdown-item" onClick={handleSortByNearToEnd}>
            {" "}
            Near to End
          </a>
        </div>
